...
def test_sc_basic():
    ...
    assert 4 == len(result)
    assert set([0, 2, 4, 6])== result

def test_sc_nested():
    result = { x+y for x in range(3) for y in range(3)}
    assert 5 == len(result)
    assert set([0, 1, 2, 3, 4])== result

def test_sc_conditional():
    result = { x**2 for x in range (5) if x % 2 == 1}
    assert 2 == len(result)
    assert set([1, 9]) == result
...
def test_sc_filtering():
    ...
    assert set([1, 3, 9, 5, 7]) == odds


three_things_i_learnt = """
-syntax and creation and list comprehension
-syntax and creation of dict comprehension
-syntax and creation of set comprehension
"""
