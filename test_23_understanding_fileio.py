...
def test_file_tell():
    ...
    while True:
        ...
        if not line:
            break
    assert [5L, 10L, 17L, 23L, 27L, 27L] == tells
...
def test_file_readlines_tell():
    ...
    for line in f.readlines():
        tells.append(f.tell())
    assert [27L, 27L, 27L, 27L, 27L] == tells
...
def test_file_iteration_tell():
    ...
    for line in f:
        tells.append(f.tell())
    assert [27L, 27L, 27L, 27L, 27L]== tells # is there really no difference between readlines and iteration?
...
-files and their modes
-various methods on files like seek,tell
-accessing files
"""
