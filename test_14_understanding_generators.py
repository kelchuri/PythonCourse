...
def test_range_allocates_memory():
    try:
        for item in range(1000 * (10**6)):
            if item%5 == 1:
                break
    except MemoryError as me:  # what error do you get when you allocate 1 billion ints?
        assert True
...
def demo_generator_to_list(generator):
    seq_generator = iter(generator)
    result = []
    try:
        while True:
            item = seq_generator.next()
            result.append(item)
    except StopIteration as se:
        return result
...
-defintion and use of generators
