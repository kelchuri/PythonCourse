...
def test_set_empty():
    ...
    assert False == isinstance(empty_set_wrong, set) #it can be clarified only when the values are metioned
...
def test_set_length():
    ...
    assert 1 == len(fruits)  #are duplicates removed? # yes ! they are removed
...
-definition and use of sets (there is no particular order for printing , might be LIFO)
